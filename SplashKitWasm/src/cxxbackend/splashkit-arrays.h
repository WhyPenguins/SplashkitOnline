// Copied from newer version of SplashKit, remove later once we update.
...
class fixed_array
{
    ...
    void check_index(int index, const std::string& access_type) const
    {
        if (index < 0 || index >= _size)
        {
            if (_size == 0)
            {
                write_line("Cannot access index " + std::to_string(index) +
    " because array is empty.");
            }
            else
            {
                write_line("Index to " + access_type + " (" + std::to_string(index) + ") is outside of range 0 - " + std::to_string(_size - 1) + ".");
            }
...
class dynamic_array
{
    ...
    " because array is empty.");
            ...
            {
                write_line("Index to " + access_type + " (" + std::to_string(index) + ") is outside of range 0 - " + std::to_string(static_cast<int>(data.size()) - 1) + ".");
            }
            throw array_invalid_index();
        }
    }
    ...
    void check_insert_index(int index) const
    {
        if (index < 0 || index > static_cast<int>(data.size()))
        {
            write_line("Index to insert (" + std::to_string(index) + ") is outside of range 0 - " + std::to_string(static_cast<int>(data.size())) + ".");
            throw array_invalid_index();
        }
    }
    ...
    void resize(int new_size)
    {
        if (new_size < 0)
        {
            write_line("Invalid dynamic_array size (" + std::to_string(new_size) + "). Size must be 0 or greater.");
