  ...
  switch (data.target) {
    ...
    case 'Image': {
      ...
      img.onload = () => {
        ...
        sendWorkerCommand("EmEvent", { target: 'Image', method: 'onload', id: data.id, width: img.width, height: img.height, data: imageData.data, preMain: true });
      };
      img.onerror = () => {
        sendWorkerCommand("EmEvent", { target: 'Image', method: 'onerror', id: data.id, preMain: true });
      };
      img.src = data.src;
      break;
    }
...
['keydown', 'keyup', 'keypress', 'blur', 'visibilitychange'].forEach((event) => {
  document.addEventListener(event, (event) => {
    sendWorkerCommand("EmEvent", { target: 'window', event: cloneObject(event) });
...
['mousedown', 'mousemove', 'DOMMouseScroll', 'mousewheel', 'mouseout'].forEach((event) => {
  Module.canvas.addEventListener(event, (event) => {
    sendWorkerCommand("EmEvent", { target: 'canvas', event: cloneObject(event) });
    event.preventDefault();
  }, true);
});
